        ...
        <div className="mt-4 grid grid-cols-1">
          <div className="bg-white rounded-2xl p-4 border border-gray-200">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2"><LinkIcon className="h-4 w-4"/> Invite Friends</h3>
              {inviteCode && (
                <span className="text-xs px-2 py-1 rounded bg-gray-100 text-gray-700">Your code</span>
              )}
            </div>
        ...
        {showFollowModal && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-6 w-full max-w-md border border-gray-200">
              <h3 className="text-lg font-bold text-gray-900 mb-2">Follow via Code</h3>
              <input
                value={followInput}
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={()=>setShowFollowModal(false)} className="flex-1 px-4 py-2 border border-gray-300 rounded-xl">Cancel</button>
                <button
                  onClick={() => {
                    const norm = normalizeCode(followInput);
        ...
        <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600">Current Goal Progress</span>
                ...
                <div className="space-y-4">
                  <div className="bg-green-50/20 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-green-700">{stats.goalsCompleted}</p>
                  </div>
                  
                  <div className="bg-blue-50/20 rounded-lg p-4">
                    <p className="text-sm text-blue-600">Best Streak</p>
                    <p className="text-2xl font-bold text-blue-700">{stats.bestStreak} days</p>
                  </div>
                  
                  <div className="bg-purple-50/20 rounded-lg p-4">
                    <p className="text-sm text-purple-600">Days Active</p>
                    <p className="text-2xl font-bold text-purple-700">{stats.daysActive}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {achievementBadges.map((achievement, index) => {
                  const categoryColors: Record<string, string> = {
                    progress: 'from-blue-50 to-blue-100 border-blue-200/20/20',
                    goals: 'from-green-50 to-green-100 border-green-200/20/20',
                    streaks: 'from-orange-50 to-red-100 border-orange-200/20/20',
                    discipline: 'from-purple-50 to-purple-100 border-purple-200/20/20',
                    growth: 'from-emerald-50 to-emerald-100 border-emerald-200/20/20',
                    special: 'from-yellow-50 to-yellow-100 border-yellow-200/20/20',
                    leaderboard: 'from-rose-50 to-pink-100 border-rose-200/20/20'
                  };
                  
                  return (
                    <div key={index} className={`bg-gradient-to-br ${categoryColors[achievement.category]} border rounded-lg p-3 text-center`}>
                      <div className="text-2xl mb-1">{achievement.icon}</div>
                      <h4 className="font-semibold text-xs text-gray-900">{achievement.name}</h4>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              {(() => {
                ...
                if (!history || history.length === 0) {
                  return (
                    <p className="text-sm text-gray-600">No monthly history yet. Complete a cycle to see your badges here.</p>
                  );
                }
                      ...
                      return (
                        <div key={idx} className="flex items-center justify-between p-3 rounded-xl border border-gray-200">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-lg bg-rose-50/20 flex items-center justify-center text-xl">
                              {badge.icon || '📊'}
                            </div>
                            <div>
                              <p className="font-semibold text-gray-900">{m.month}</p>
                              <p className="text-xs text-gray-600">Your Rank: {m.yourRank > 0 ? `#${m.yourRank}` : '—'}</p>
                            </div>
                          </div>
                          {badge.icon && (
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className="space-y-4">
                <div className="bg-blue-50/20 rounded-lg p-4">
                  <h4 className="font-semibold text-blue-700 mb-2">Trading Style</h4>
                ...
                <div className="bg-green-50/20 rounded-lg p-4">
                  <h4 className="font-semibold text-green-700 mb-2">Consistency</h4>
                ...
                <div className="bg-purple-50/20 rounded-lg p-4">
                  <h4 className="font-semibold text-purple-700 mb-2">Growth Focus</h4>
